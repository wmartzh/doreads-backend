  ...
  *  @param {Response} res
  ...
  async createStudent(req: Request | any, res: Response) {
    try {
      ...
      this.responseHandler(res, { message: `Student ${result.name} registered successfully` }, 200);
    } catch (error: any) {
  ...
  async getAllStudents(req: Request | any, res: Response) {
    try {
      ...
      const result = await studentService.getAllStudents(limit, offset, filter, search);
