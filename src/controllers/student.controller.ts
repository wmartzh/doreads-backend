...
class StudentController extends BaseController {
  ...
  async getStudentById(req: Request | any, res: Response) {
    try {
      ...
      this.responseHandler(
        res,
        await studentService.getStudentById(Number(id)),
        200
      );
    } catch (error: any) {
      this.errorHandler(res, { error: "Student doesn't exist" });
  ...
  async deleteStudentById(req: Request | any, res: Response) {
    try {
      ...
      this.responseHandler(
        res,
        await studentService.deleteStudentById(Number(id), req.user.role),
        200
      );
    } catch (error: any) {
      if (error.code && error.code === "P2025") {
  ...
  async updateStudent(req: Request | any, res: Response) {
    try {
      ...
      this.responseHandler(
        res,
        await studentService.updateStudent(studentData, Number(id)),
        200
      );
    } catch (error: any) {
      if (error.code && error.code === "P2025") {
